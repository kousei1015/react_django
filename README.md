...
1. ログイン状態でなくても、投稿が見られるように変更しました。
2. ソート機能を使う際の不具合を修正しました。(投稿全体がソートされず、そのページ内の投稿のみでソートされてしまっていたのを修正しました。)
3. Coreコンポーネントからナビゲーションバーを作るコンポーネントを分けることにしました。今まではトップページにあたるCoreコンポーネントにナビゲーションバーも書いていました。しかし、これでは可読性もあまり良くありません。また、ナビゲーションバーの部分はほとんどstateが変わることもないですが、Coreコンポーネントはページネーション機能の関係上、そこそこの頻度で再レンダリングされてしまいます。そのため、ナビゲーションバーの部分はパフォーマンス的にもNavbarコンポーネントとして分けて、memoで囲んだ方が適切だと感じ、Coreコンポーネントからナビゲーションバーを作るコンポーネントを分けることにしました。
